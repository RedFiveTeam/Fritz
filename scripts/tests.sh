...
function acceptanceTests {
    ...
    java -DCLASSIFIED=UNCLASSIFIED -DUNICORN_URL=https://codweb1.leidoshost.com/UNICORN.NET -jar ${BASE_DIR}/target/fritz-[0-9\.]*-SNAPSHOT.jar --server.port=9090 &> ${BASE_DIR}/tmp/acceptance.log &
...
function unitTests {
    ...
    pushd ${BASE_DIR}
        result=$(mvn -DCLASSIFIED=UNCLASSIFIED -DUNICORN_URL=https://codweb1.leidoshost.com/UNICORN.NET test | grep -E "\[INFO\]|\[ERROR\]|Expected")
        echo "${result}"
